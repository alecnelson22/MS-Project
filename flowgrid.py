...
class CMG(FlowGrid):
    ...
    def vtp(self, f_vtp, t):
        ...
        geometryFilter.SetInputData(self.Grid)
        geometryFilter.Update()
        polydata = geometryFilter.GetOutput()

        # For time = 0, we export polydata (vtp) file
        # This provides us with a mesh that we can then just update data for
        dp = []
        # Determine static properties
        for prop in self.out_props:
            if type(self.out_props[prop]) is not dict:
                if t == 0:
                    cell_data = polydata.GetCellData().GetArray(prop)
                    np_data = vtk_to_numpy(cell_data)
                    self.export_JSON(np_data, prop, t)
            else:
                dp.append(prop)

        if t == 0:
            writer =vtk.vtkXMLPolyDataWriter()
            writer.SetFileName(os.path.join(self.out_dir, f_vtp + '.vtp'))
                ...
                for line in fp:
                    ...
                    if len(item) > 0:
                        ...
                        if build:
                            ...
                            # Put entire grid worth of property in dictionary for current time step
                            if I is not None and J is not None:
                                if int(I[-1]) == self.size[0] and int(JIdx) == self.size[1] and int(K) == self.size[
                                    2]:
                                    if build:
                                        self.out_props[attr_title][time] = layers
                                        build = False
    ...
    class Wells:
        ...
        def build_cylinders(self, vtk_fname, zscale=1, radius=20):
            ...
            fp = os.path.join(self.out_dir, 'well_cylinders.py')
            with io.open(fp, 'w', newline='\r\n') as f:
    ...
    def export_grid(self, vtk_fname='GRID', toVTK=True, toNumpy=True, toJSON=False):
        """
        ...
        for prop in self.out_props:
            if type(self.out_props[prop]) is not dict:
                ...
                if toJSON:
                    self.export_JSON(data, prop, tID)
    ...
    def export_JSON(self, d, title, t):
        self._check_out('JSON')
        f_path = os.path.join(self.out_dir, 'JSON', title + '_' + str(t) + '.json')

        jd = d.flatten(order='F').tolist()
        json.dump(jd, codecs.open(f_path, 'w', encoding='utf-8'), separators=(',', ':'), sort_keys=True, indent=4)
